...
export function cloneRecord(card, native, now = new Date()) {
  return {
...
export function selectForClone(cards, { categories = [], levels = [], skip = new Set() } = {}) {
...
export function planClone({ native, from, to, categories = [], levels = [] } = {}) {
  ...
  if (from === to) throw new TypeError('a deck cannot be cloned onto itself');
  if (home === to) throw new TypeError('the language you write in cannot also be a deck you learn');

  const source = db.deckIdIfAny(home, from);
  if (source === null) throw new Error(`no ${home}>${from} deck to copy from`);

  const destination = db.deckId(home, to);
  const skip = db.originsOfDeck(destination);
  for (const row of readJsonl(queueFile(to))) if (typeof row.origin === 'string') skip.add(row.origin);
  const wanted = selectForClone(db.cardsOfDeck(source), { categories, levels, skip });

  const now = new Date();
  const records = wanted.map((card) => cloneRecord(card, home, now));
  if (records.length) appendJsonl(queueFile(to), records);
  log(`clone ${JSON.stringify({ from, to, queued: records.length, skipped: skip.size })}`);

  return { from, to, native: home, queued: records.length, skipped: skip.size, pending: readJsonl(queueFile(to)).length };
}
